...
function salvaProgramador() {
    ...
    $.ajax({
            ...
            data: {
                vnome__programador: nome,
                vlogin_programador: login,
                vsenha_programador: senha,
                vsenha_programador1: repitasenha
            },
            encode: true,
        })
    ...
    $.ajax({
        ...
        .done(function (data) {
            ...
            for (var i = 0; i < length; i++) {
                ...
                var p =data.message[i][data.message[i].length -1].split('/');
                var final = new Date();
                final.setFullYear(p[2],p[1] -1,p[0])
                var today = new Date();
                if(final < today){
                    console.log(theNode);
                    theNode.style.backgroundColor = "#F87357";
                }
